...
def token_key_frame(
    ...
    tokens = (
        ...
        .with_columns(pl.col("business_name_normalized").str.split(" ").alias("key"))
        .explode("key", empty_as_null=True)
        .filter(pl.col("key").str.len_chars() >= min_length)
        .unique(subset=["entity_id", "key"])
    )
    if include_country:
...
def profile_key_blocks(
    ...
    left = left_keys.group_by("key").len().rename({"len": "s1_count"})
    right = right_keys.group_by("key").len().rename({"len": "target_count"})
    ...
    summary["unique_key_count"] = (
        left.select(pl.col("key").n_unique()).collect(engine="streaming").item()
        + right.select(pl.col("key").n_unique()).collect(engine="streaming").item()
        - shared_count
    )
    summary["record_count"] = (
        left.select(pl.col("s1_count").sum()).collect(engine="streaming").item()
        + right.select(pl.col("target_count").sum()).collect(engine="streaming").item()
    )
    if pair_cap is not None:
        summary["pair_cap"] = pair_cap
        summary["eligible_shared_key_count"] = shared.filter(
            pl.col("pair_estimate") <= pair_cap
        ).select(pl.len()).collect(engine="streaming").item()
        summary["eligible_estimated_candidate_pairs"] = shared.filter(
            pl.col("pair_estimate") <= pair_cap
        ).select(pl.col("pair_estimate").sum()).collect(engine="streaming").item()
    else:
    ...
    top = (
        shared.sort("pair_estimate", descending=True)
